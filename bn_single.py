...
with tf.Session() as sess:
    ...
    for i in range(5000):
        train_images, train_labels = train_data.next_batch(50)
        # train_images = train_data.images
        # train_labels = train_data.labels
        if i % 100 == 0:
            train_accuracy = accuracy.eval(feed_dict={
                x: train_images, y_: train_labels, keep_prob: 1.0, phase: True})
            print (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
            print('step %d, training accuracy %g' % (i, train_accuracy))
            print('test accuracy %g' % accuracy.eval(feed_dict={
                x: test_data.images, y_: test_data.labels, keep_prob: 1.0, phase: False}))
            print('train loss %g' % cross_entropy.eval(feed_dict={
                x: train_data.images, y_: train_data.labels, keep_prob: 1.0, phase: True}))
            print('test loss %g' % cross_entropy.eval(feed_dict={
                x: test_data.images, y_: test_data.labels, keep_prob: 1.0, phase: False}))
        train_step.run(feed_dict={x: train_images, y_: train_labels, keep_prob: 0.5, phase: True})
    print('test accuracy %g' % accuracy.eval(feed_dict={
        x: test_data.images, y_: test_data.labels, keep_prob: 1.0, phase: False}))
    save_path = saver.save(sess, "./bn_single", global_step=5000)
