...
imgs_batch, label_batch = dp.batch('./tfrecords/train.tfrecords', True, frame_size, img_height, img_width, batch_size)
imgs_batch_test, label_batch_test = dp.batch('./tfrecords/test.tfrecords', False, frame_size, img_height, img_width, batch_size)s
...
with tf.Session() as sess:
    ...
    for i in range(3000):
        ...
        batch_x, batch_y = sess.run([imgs_batch, label_batch])
        batch_x = np.array(batch_x)
        batch_x = batch_x.reshape(batch_size, -1)
        batch_y = np.array([labels_onehot_dict[l] for l in batch_y], dtype=int)

        batch_x_test, batch_y_test = sess.run([imgs_batch_test, label_batch_test])
    ...
    graph = tf.graph_util.convert_variables_to_constants(sess, sess.graph_def, ["y_conv"])
    tf.train.write_graph(graph, './models/3d-model', '3d.pb', False)
