...
def deepnn(x):
    ...
    W_conv1 = weight_variable([12, filter_size1, filter_size1, channel_num, filter_num1])
    b_conv1 = bias_variable([filter_num1])
    conv1 = tf.nn.conv3d(x_video, W_conv1, strides=[1, 12, stride1, stride1, 1], padding='SAME')
    ...
    W_conv2 = weight_variable([1, filter_size2, filter_size2, filter_num1, filter_num2])
    b_conv2 = bias_variable([filter_num2])
    conv2 = tf.nn.conv3d(h_pool1, W_conv2, strides=[1, 1, stride2, stride2, 1], padding='SAME')
    ...
    W_conv3 = weight_variable([1, filter_size3, filter_size3, filter_num2, filter_num3])
    b_conv3 = bias_variable([filter_num3])
    conv3 = tf.nn.conv3d(h_pool2, W_conv3, strides=[1, 1, stride3, stride3, 1], padding='SAME')
...
x = tf.placeholder(tf.float32, [None, video_size_flat])
...
with tf.Session() as sess:
    ...
    for i in range(5000):
        train_videos, train_labels = train_data.next_batch(50)
        ...
        if i % 100 == 0:
            ...
            print('test loss %g' % cross_entropy.eval(feed_dict={
                x: test_data.videos, y_: test_data.labels, keep_prob: 1.0, phase: False}))
        train_step.run(feed_dict={x: train_videos, y_: train_labels, keep_prob: 0.5, phase: True})
    print('test accuracy %g' % accuracy.eval(feed_dict={
        x: test_data.videos, y_: test_data.labels, keep_prob: 1.0, phase: False}))
